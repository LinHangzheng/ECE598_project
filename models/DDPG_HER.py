...
from lib.HER_sampler import her_sampler
...
class DDPG_HER(object):
    def __init__(self, args, env):
        ...
        self.normalizer_obs = normalizer(size=self.env_params['observation'], default_clip_range=200)
        self.normalizer_goal = normalizer(size=self.env_params['goal'], default_clip_range=200)

        self.her_sampler = her_sampler(replay_strategy='future',replay_k=4,reward_func=self.env.compute_reward)
    ...
    def _update_model(self):
        ...
        for epoch in tqdm(range(self.args.epoch_num)):
            ...
            HER_transition = self.her_sampler.sample_her_transitions(transition, 128)
            # print(HER_transition['desired_goal'].shape)
            # print(HER_transition['obs'].shape)
            
            self.normalizer_obs.update(HER_transition['obs'])
            self.normalizer_goal.update(HER_transition['desired_goal'])
            # recompute the stats
            self.normalizer_obs.recompute_stats()
            self.normalizer_goal.recompute_stats()


            obss = torch.from_numpy(HER_transition['obs']).float().to(self.device)
            obss = np.clip(obss, -200, 200)
            obss = self.normalizer_obs.normalize(obss)

            next_obss = torch.from_numpy(HER_transition['next_obs']).float().to(self.device)
            next_obss = np.clip(next_obss, -200, 200)
            next_obss = self.normalizer_obs.normalize(next_obss)

            goals = torch.from_numpy(HER_transition['desired_goal']).float().to(self.device)
            goals = np.clip(goals, -200, 200)
            goals = self.normalizer_goal.normalize(goals)
            ...
            states = torch.cat((obss,goals),1).float().to(self.device)
            next_states = torch.cat((next_obss,goals),1).float().to(self.device)
